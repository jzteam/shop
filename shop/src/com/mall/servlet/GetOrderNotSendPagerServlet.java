...
public class GetOrderNotSendPagerServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		OrderNotSendPager osp = model.getOrderNotSendPager(offset, pageSize);
		if(osp.getOrderMap().size() == 0 && offset != 0) {
			offset -=pageSize;
			osp = model.getOrderNotSendPager(offset, pageSize);
		}
		...
		request.getRequestDispatcher("Admin/pages/manageNotSendOrder.jsp").forward(request, response);
		}else{
