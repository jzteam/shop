...
public class GetGoodsByTypeServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 根据查询类型，查询商品
        List showGoods = model.showGoods(Integer.parseInt(type), 1);

        if (showGoods != null && showGoods.size() > 6) {
            showGoods = showGoods.subList(0, 6);
        }

        ResultJsonUtil.success(response, showGoods);
