...
public class OrderDaoImpl implements OrderDao {
	...
	public int addOrder(Order order) {
		...
		ResultSet rs = null;
		DbUtil dbUtil = null;
		String sql_order = "insert into tb_order values(?,?,?,?,?,getdate(),?)";
		try {
			//事物处理
			dbUtil = new DbUtil();
			dbUtil.getCon().setAutoCommit(false);
			//将订单中的数据录入数据库
			pstmt_order = dbUtil.getCon().prepareStatement(sql_order,Statement.RETURN_GENERATED_KEYS);
			...
			//将订单项中的数据录入数据库
			String sql_item = "insert into tb_orderItem values(?,?,?,?,?)";
